...
const skills = [
  ...
  { name: "C#", level: 50, category: "frontend" },
  { name: "Java", level: 40, category: "frontend" },

  // Backend
  { name: "Node.js", level: 80, category: "backend" },
  { name: "Express", level: 75, category: "backend" },
  { name: "MongoDB", level: 70, category: "backend" },
  { name: "DrizzleORM", level: 60, category: "backend" },
...
export const SkillsSection: React.FC = () => {
  const [activeCateory, setActiveCategory] = useState("all");

  const filteredSkills = skills.filter((skill) => {
    if (activeCateory === "all" || skill.category === activeCateory)
      return true;
    return false;
  });
